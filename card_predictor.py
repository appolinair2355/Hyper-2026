# card_predictor.py - VERSION ULTRA ROBUSTE & PROTÉGÉE
import re
import logging
import time
import os
...
class CardPredictor:
    def __init__(self, telegram_message_sender=None):
        # <<<<<<<<< ZONE À MODIFIER PAR L'UTILISATEUR >>>>>>>>>
        ...
        # --- Chargement des données avec PROTECTION ANTI-ERREUR ---
        ...
        # Trackers temporels avec valeurs par défaut sûres
        ...
        # Auto-activation si données disponibles
        if len(self.inter_data) >= 3 and not self.is_inter_mode_active:
            logger.info(f"📊 Auto-activation INTER: {len(self.inter_data)} jeux disponibles")
            self.analyze_and_set_smart_rules(initial_load=True)
        
        logger.info(f"✅ CardPredictor initialisé - {len(self.inter_data)} jeux, {len(self.smart_rules)} règles")

    # --- PERSISTENCE ROBUSTE avec PROTECTION CONTRE LES ERREURS DE TYPE ---
    def _load_data(self, filename: str, is_set: bool = False, is_scalar: bool = False, 
        ...
        """
        Chargement SÉCURISÉ des données avec protection contre les erreurs de type
        GARANTIT que les dicts restent des dicts et les listes des listes
        """
        try:
    ...
    def _save_data(self, data: Any, filename: str):
        ...
        try:
            ...
            # Log pour debug
            logger.debug(f"💾 Sauvegarde {filename} (type: {type(data).__name__})")
    ...
    def should_predict(self, message: str) -> Tuple[bool, Optional[int], Optional[str], Optional[bool]]:
        ...
        if any(p.get('status') == 'pending' for p in self.predictions.values()):
            logger.warning(f"⚠️ Prédiction déjà en attente")
            return False, None, None, None
        ...
        if self.is_inter_mode_active and self.smart_rules:
            ...
            if not predicted_suit:
                ...
                logger.error(f"Règles: {[normalize_card(r['trigger']) for r in self.smart_rules]}")
    ...
    def _mark_rule_as_used(self, trigger: str, suit: str):
        ...
        self.quarantined_rules[suit][trigger] = self.quarantined_rules.get(suit, {}).get(trigger, 0) + 1
    ...
    def _check_suit_repetition(self, suit: str) -> bool:
        ...
        return count < 2  # Max 2 répétitions consécutives
