...
def run_inter_analysis():
    ...
    try:
        if telegram_bot and hasattr(telegram_bot, 'handlers') and telegram_bot.handlers.card_predictor:
            ...
            # Envoyer notification de mise à jour réussie au bot lui-même (en utilisant son propre ID extrait du token)
            token = os.getenv('BOT_TOKEN')
            bot_id = token.split(':')[0] if token and ':' in token else None
            if bot_id and predictor.telegram_message_sender:
                now = datetime.now(pytz.timezone('Africa/Porto-Novo'))
                ...
                predictor.telegram_message_sender(bot_id, msg)
                logger.info(f"📢 Notification de mise à jour envoyée au bot lui-même (ID: {bot_id})")
            else:
                logger.warning("⚠️ Impossible de déterminer l'ID du bot pour la notification.")
    except Exception as e:
